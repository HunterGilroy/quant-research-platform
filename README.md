...
An end-to-end systematic equity research platform with automated daily paper trading. Every weekday, GitHub Actions scans the S&P 500 on a walk-forward-weighted signal blend, runs a tournament between four portfolio-construction methods (mean-variance, risk parity, HRP, inverse-vol), applies a volatility-targeting overlay, and trades an Alpaca paper account to target — then commits its own research report to this repo.

**Research mode (current configuration):** this platform is the rule-based side of a controlled head-to-head study against an AI-driven trading agent. Both systems trade the identical fixed S&P 500 universe with identical starting capital ($100,000 paper), and every order and every no-trade decision is logged to CSV (`output/trades.csv`, `output/decisions.csv`). For the study, the single-name concentration cap is disabled — position sizing is left entirely to the strategy.
# How It Works
Every weekday at 10:45 AM ET, GitHub Actions runs the full research-to-execution pipeline with no human involvement:
1. Universe & data. The platform trades a fixed, built-in list of S&P 500 constituents (`qrp/universe.py` — a snapshot taken at the start of the study, identical to the AI agent's universe so results are directly comparable) and downloads two years of daily prices for all ~500 stocks via yfinance.
2. Signal scoring. Every stock is scored on a blend drawn from seven classic cross-sectional signals — 12-1 momentum (Jegadeesh & Titman, 1993), 50/200-day trend, 1-month reversal, low volatility, 52-week-high proximity (George & Hwang 2004), the MAX/lottery effect (Bali et al. 2011), and betting-against-beta (Frazzini & Pedersen 2014). Signal weights are chosen walk-forward each day from trailing rank IC, using only past data. Scores are winsorized and z-scored so signals are comparable. The top ~20 names by combined score become the candidate portfolio.
...
Signal weights are selected walk-forward from trailing IC, which removes the worst in-sample reasoning but still assumes recent signal efficacy persists.
