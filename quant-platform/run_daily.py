...
def build_target(prices: pd.DataFrame):
    ...
    tgt = tgt / tgt.sum()
    # ...THEN vol targeting (a risk decision). Order matters: scaling shrinks
    # every weight, and filtering after scaling can delete the entire
    # portfolio on a high-vol day when the intent was reduced exposure.
    rm = RiskModel(n_factors=5).fit(rets[tgt.index])
