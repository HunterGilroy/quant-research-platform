...
class SimulatedBroker:
    ...
    def submit_order(self, symbol: str, qty: float, side: str, price: float) -> dict:
        ...
        fill = min(price + slip, limit) if side == "buy" else max(price + slip, limit)
...
class AlpacaBroker:
    ...
    def submit_order(self, symbol: str, qty: float, side: str, price: float) -> dict:
        """Marketable limit order: priced 0.1% through the reference quote, so
        it fills like a market order but slippage is capped instead of
        unbounded. Unfilled orders expire at day end; the next idempotent run
        re-diffs and re-submits, so nothing is lost."""
        buffer = 0.003   # small-cap spreads are wider
        limit = price * (1 + buffer) if side == "buy" else price * (1 - buffer)
        ...
        return {"symbol": symbol, "qty": qty, "side": side,
                "limit_price": round(limit, 2), "order_id": str(o.id)}
