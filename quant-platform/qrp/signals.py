...
# ----------------------------------------------------------------------
@register("high_52w")
def high_52w(prices: pd.DataFrame) -> pd.Series:
    """52-week-high proximity: stocks near their 52-week high tend to keep
    outperforming — anchoring makes investors slow to bid them up.
    George & Hwang (2004), Journal of Finance."""
    if len(prices) < 252:
        return pd.Series(0.0, index=prices.columns)
    ratio = prices.iloc[-1] / prices.iloc[-252:].max()
    return zscore_xs(ratio)

@register("lottery")
def lottery(prices: pd.DataFrame) -> pd.Series:
    """MAX effect: stocks with extreme recent daily gains ('lottery tickets')
    subsequently underperform — investors overpay for jackpot-like payoffs.
    High score = boring, no recent moonshots.
    Bali, Cakici & Whitelaw (2011), Journal of Financial Economics."""
    if len(prices) < 22:
        return pd.Series(0.0, index=prices.columns)
    mx = prices.pct_change().iloc[-21:].max()
    return zscore_xs(-mx)

@register("low_beta")
def low_beta(prices: pd.DataFrame) -> pd.Series:
    """Betting against beta: low-beta stocks earn more per unit of risk than
    the CAPM says they should, because leverage-constrained investors bid up
    high-beta stocks instead. High score = low beta vs the universe.
    ...
    var_m = mkt.var()
    beta = rets.apply(lambda col: col.cov(mkt)) / (var_m + 1e-12)
    return zscore_xs(-beta)
